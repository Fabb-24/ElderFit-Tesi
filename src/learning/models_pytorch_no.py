...
def evaluate_model(model, X1_test, X2_test, y_test, custom_device=None):
    """
...
def objective(trial, X1_train, X2_train, y_train, X1_val, X2_val, y_val, num_classes):
    ...
    hyperparameters = {
        ...
        'hidden_size_3': trial.suggest_int('hidden_size_3', 16, 96, step=32),
    ...
    if val_accuracy > best_accuracy:
        ...
        save_model(model, os.path.join(util.getModelsPath(), 'LSTM_Combo3_full.pth'))
...
def train_best_model(best_param, X1, X2, y, X1_val, X2_val, y_val, num_classes, save_path):
    ...
    evaluate_model(model, X1_val, X2_val, y_val)  # Valutazione del modello
    save_model(model, os.path.join(save_path, 'LSTM_Combo3_full_opt.pth'))  # Salvataggio del modello
...
def create_model(X1, X2, X3, y, X1_test, X2_test, X3_test, y_test, num_classes):
    ...
    study.optimize(lambda trial: objective(trial, X1, X3, y, X1_test, X3_test, y_test, num_classes), n_trials=50)
    best_params = study.best_params
    print(f"\nBest params: {best_params}")
    best_params['X1_size'] = X1.shape[2]
    best_params['X2_size'] = X3.shape[2]
    #best_params['X3_size'] = X3.shape[2]
    best_params['num_classes'] = num_classes
    np.save(os.path.join(util.getModelsPath(), 'best_params_noof.npy'), best_params)
...
def create_model_2(X1, X2, y, X1_test, X2_test, y_test, num_classes):
    """
    Funzione che crea il modello e ne ottimizza gli iperparametri.
    ...
    best_params['num_classes'] = num_classes
    np.save(os.path.join(util.getModelsPath(), 'best_params_full.npy'), best_params)
    print("Salvati in 'best_params.npy'")
    #print("\nAddestramento del modello con i migliori iperparametri...")
    #train_best_model(best_params, X1, X2, X3, y, X1_test, X2_test, X3_test, y_test, num_classes, util.getModelsPath())
