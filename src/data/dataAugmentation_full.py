...
class Videos:
    ...
    def get_videos(self):
        ...
        for exercise in os.listdir(self.folder):
            videos[exercise] = []
            for type in os.listdir(os.path.join(self.folder, exercise)):
                for video in os.listdir(os.path.join(self.folder, exercise, type)):
                    videos[exercise].append((os.path.join(self.folder, exercise, type, video)))
    ...
    def process_videos(self):
        ...
        for exercise in videos:
            for video_path in tqdm.tqdm(videos[exercise], desc=f"Modify speed of {exercise} videos", unit="video"):
        ...
        for exercise in videos:
            for video_path in tqdm.tqdm(videos[exercise], desc=f"Processing {exercise} videos", unit="video"):
                #video_path = os.path.join(self.folder, exercise, video)
